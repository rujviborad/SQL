...
SELECT * FROM STUDENT 
WHERE  ( SPI >7.00 AND SPI <=9.00 ) AND  (STDID BETWEEN 102 AND 108)
...
SELECT * FROM STUDENT 
WHERE BRANCH IN('CIVIL' , 'MECHANICAL') AND SPI > 8.00
...
SELECT * FROM STUDENT 
WHERE NOT SPI BETWEEN 6.00 AND 8.00
...
SELECT * FROM STUDENT 
WHERE (SPI>8.00) AND NOT BRANCH IN('COMPUTER','CIVIL') 
...
SELECT * FROM STUDENT 
WHERE (SPI > 7.5 AND SPI < 9.00) AND NOT  STDID  BETWEEN 104 AND 108
...
--16. Retrieve student name and id along with the city whose city is RAJKOT and SPI is less than 8.0,
--or whose city is SURAT and SPI is greater than 8.0.

SELECT SNAME , STDID , CITY FROM STUDENT 
WHERE (CITY='RAJKOT' AND SPI<8.0 ) OR (CITY='SURAT' AND SPI>8.0 )


--17. Display students who belong to COMPUTER branch and have SPI greater than 8.0, or belong to CIVIL 
--branch and have SPI greater than 7.0.

SELECT * FROM STUDENT 
WHERE (BRANCH ='COMPUTER' AND SPI>8.00) OR (BRANCH ='CIVIL' AND SPI>7.00)
